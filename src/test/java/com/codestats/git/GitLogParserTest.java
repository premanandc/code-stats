...
class GitLogParserTest {
    ...
    void shouldFilterCommitsByDateRange() {
        ...
        assertThat(filtered).extracting(GitCommit::hash)
            .containsExactly("hash2", "hash3");
    }
    ...
    void shouldFilterCommitsByAuthorEmails() {
        ...
        assertThat(filtered).hasSize(2);
        assertThat(filtered).extracting(GitCommit::authorEmail)
    ...
    void shouldGroupFileChangesByLanguage() {
        List<FileChange> fileChanges = List.of(
        ...
        GitCommit commit = new GitCommit(
            "hash123", "dev", "dev@example.com", LocalDateTime.now(),
            "Test commit", fileChanges, 38, 8
        );

        Map<String, String> langMapping = Map.of(
            "java", "Java",
            "py", "Python", 
            "md", "Markdown"
        );
    ...
    private GitCommit createCommit(String hash, String email, LocalDateTime date) {
        return new GitCommit(
            hash, "Test Author", email, date, "Test message",
            List.of(), 10, 5
        );
    }
}