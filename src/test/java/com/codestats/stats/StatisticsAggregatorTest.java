...
class StatisticsAggregatorTest {
    ...
    void shouldAggregateStatsForSingleContributor() {
        ...
        List<GitCommit> commits = List.of(
            createCommit("Alice", "alice@company.com", 
                List.of(
            ...
            createCommit("Alice", "alice@company.com",
                List.of(
                    new FileChange("src/Auth.java", 8, 0, FileChange.ChangeType.ADDED)
                ), 8, 0)
        );

        // Create identity mapping
        Map<String, ContributorIdentity> identities = Map.of(
            "alice@company.com", ContributorIdentity.builder()
        ...
        List<ContributorStats> stats = aggregator.aggregateStatistics(
    ...
    void shouldSeparateProductionAndTestLines() {
        List<GitCommit> commits = List.of(
            createCommit("Bob", "bob@dev.com",
                List.of(
                    new FileChange("src/main/java/Service.java", 20, 5, FileChange.ChangeType.MODIFIED),
                    new FileChange("src/test/java/ServiceTest.java", 10, 2, FileChange.ChangeType.MODIFIED),
                    new FileChange("docs/README.md", 5, 0, FileChange.ChangeType.MODIFIED)
                ), 35, 7)
        );

        Map<String, ContributorIdentity> identities = Map.of(
            "bob@dev.com", ContributorIdentity.builder()
                .canonicalName("Bob")
    ...
    void shouldHandleMultipleContributors() {
        ...
        List<ContributorStats> stats = aggregator.aggregateStatistics(
            commits, identities, Map.of(), List.of(), List.of()
        );
    ...
    void shouldGroupFileChangesByLanguage() {
        List<GitCommit> commits = List.of(
            createCommit("Dev", "dev@example.com",
                List.of(
        ...
        Map<String, ContributorIdentity> identities = Map.of(
            "dev@example.com", createIdentity("Dev", "dev@example.com")
        );
        ...
        List<ContributorStats> stats = aggregator.aggregateStatistics(
            commits, identities, languageMapping, List.of(), List.of()
        );
    ...
    void shouldIdentifyProductionVsTestCode() {
        ...
        assertThat(aggregator.isProductionCode("src/main/java/Service.java", productionPatterns, testPatterns))
            .isTrue();
        assertThat(aggregator.isProductionCode("src/test/java/ServiceTest.java", productionPatterns, testPatterns))
            .isFalse();
        assertThat(aggregator.isProductionCode("lib/utils.js", productionPatterns, testPatterns))
            .isTrue();
        assertThat(aggregator.isProductionCode("__tests__/component.spec.js", productionPatterns, testPatterns))
            .isFalse();
    ...
    // Helper methods
    private GitCommit createCommit(String author, String email, List<FileChange> changes, int insertions, int deletions) {
        return new GitCommit(
            "hash123", author, email, LocalDateTime.now(),
            "Test commit", changes, insertions, deletions
        );
    }
