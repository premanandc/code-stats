...
/**
 * Functional interface for parsing git log output into structured commit data. Pure functional
 * approach - no side effects, immutable data.
 */
public interface GitLogParser {
   ...
   */
  default List<GitCommit> filterByDateRange(
      List<GitCommit> commits, LocalDateTime since, LocalDateTime until) {
   ...
   * @return Filtered list of commits
   */
  default List<GitCommit> filterByAuthors(List<GitCommit> commits, java.util.Set<String> emails) {
    if (emails == null || emails.isEmpty()) {
      return commits;
    }

    return commits.stream().filter(commit -> emails.contains(commit.authorEmail())).toList();
  }
}
