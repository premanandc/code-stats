...
/**
 * Simple implementation of StatisticsAggregator using functional programming principles.
...
public class SimpleStatisticsAggregator implements StatisticsAggregator {
    ...
    @Override
    public List<ContributorStats> aggregateStatistics(List<GitCommit> commits,
        ...
        Map<String, List<GitCommit>> commitsByContributor = commits.stream()
            .collect(Collectors.groupingBy(
                commit -> emailToCanonical.getOrDefault(commit.authorEmail(), commit.authorEmail())
            ));
        ...
        for (Map.Entry<String, List<GitCommit>> entry : commitsByContributor.entrySet()) {
            ...
            ContributorIdentity identity = identities.getOrDefault(email, 
                createDefaultIdentity(email, contributorCommits));
            
            ContributorStats stats = aggregateContributorStats(
     ...
     */
    private ContributorStats aggregateContributorStats(ContributorIdentity identity,
     ...
     */
    private Map<String, LanguageStats> aggregateLanguageStats(List<GitCommit> commits,
                                                              Map<String, String> languageMapping) {
    ...
    private Map<String, ContributorIdentity> createDefaultIdentities(List<GitCommit> commits) {
        Map<String, List<GitCommit>> commitsByEmail = commits.stream()
            .collect(Collectors.groupingBy(GitCommit::authorEmail));
