...
/**
 * Simple implementation of AliasResolver using functional programming principles. Pure functions,
 * immutable data structures, no side effects.
 */
public class SimpleAliasResolver implements AliasResolver {

  @Override
  public Map<String, ContributorIdentity> resolveIdentities(
    ...
    Map<String, List<GitCommit>> commitsByCanonicalEmail =
        commits.stream()
            .collect(
    ...
    for (Map.Entry<String, List<GitCommit>> entry : commitsByCanonicalEmail.entrySet()) {
      ...
      ContributorIdentity identity =
          buildIdentityFromCommits(canonicalEmail, contributorCommits, aliasConfig);
  ...
  private ContributorIdentity buildIdentityFromCommits(
      String canonicalEmail, List<GitCommit> commits, Map<String, Set<String>> aliasConfig) {
