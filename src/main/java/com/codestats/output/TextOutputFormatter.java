...
public class TextOutputFormatter implements OutputFormatter {
  ...
  public String format(CodeStatsService.CodeStatsResult result) {
    ...
    output
        .append(color(CYAN, "Repository: "))
        .append(result.repositoryPath().getAbsolutePath())
        .append("\n");
  ...
  private String formatContributor(ContributorStats stats, int totalCommits, int rank) {
    ...
    sb.append("   ")
        .append(color(CYAN, "Commits: "))
        .append(stats.commitCount())
        .append(" (")
        .append(String.format("%.1f%%", percentage))
        .append(")\n");

    sb.append("   ")
        .append(color(CYAN, "Files changed: "))
        .append(stats.filesChanged())
        .append("\n");
    ...
    sb.append("   ")
        .append(color(CYAN, "Lines: "))
        .append(color(GREEN, "+" + stats.insertions()))
        .append(" ")
        .append(color(RED, "-" + stats.deletions()))
        .append(" (net: ")
        .append(color(linesColor, formatNetChange(netLines)))
        .append(")\n");
    ...
    if (!stats.languageStats().isEmpty()) {
      ...
      stats.languageStats().entrySet().stream()
          .sorted(
              (a, b) -> Integer.compare(b.getValue().linesChanged(), a.getValue().linesChanged()))
