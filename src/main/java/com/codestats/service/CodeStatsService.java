...
/**
 * Main service layer that orchestrates git analysis. Wires together all components to provide
...
public class CodeStatsService {
  ...
  public CodeStatsResult analyzeRepository(CodeStatsRequest request) {
    try {
      ...
      String gitLogOutput =
          gitCommandExecutor.executeGitLog(
      ...
      Map<String, ContributorIdentity> identities =
          aliasResolver.resolveIdentities(filteredCommits, request.config().aliases());
      ...
      return new CodeStatsResult(
          ...
          filteredCommits.isEmpty()
              ? null
              : filteredCommits.get(filteredCommits.size() - 1).commitDate(),
          true,
          null);
