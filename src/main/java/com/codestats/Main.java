...
public class Main implements Callable<Integer> {
    ...
    private CodeStatsConfig loadConfigFromFile(File configFile) throws Exception {
        if (!configFile.exists()) {
            throw new RuntimeException("Configuration file does not exist: " + configFile.getAbsolutePath());
        }
        ...
        if (aliasesObj instanceof Map) {
            ...
            for (Map.Entry<String, Object> entry : aliasesMap.entrySet()) {
                ...
                if (aliasListObj instanceof List) {
                    ...
                    Set<String> aliases = aliasList.stream()
                        .map(String::valueOf)
    ...
    private Integer handleInitConfig() {
        try {
            ...
            if (configFile.exists()) {
                ...
                System.out.println("Use --config to specify a different file name or remove the existing file.");
                return 1;
            }
            ...
            System.out.println("Edit this file to customize language mappings, directory patterns, and email aliases.");
